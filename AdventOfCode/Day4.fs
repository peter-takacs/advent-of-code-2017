...
let d4input ="""una bokpr ftz ryw nau yknf fguaczl anu
...
let rec hasAnagram row =
    ...
    | [] -> false
    | [_] -> false
    | h :: i :: t -> isAnagram h i || hasAnagram <| h::t || hasAnagram <| i::t

let withoutDuplicatesInRows table = Seq.filter isDistinct table
let withoutAnagramRows table = Seq.filter (not << hasAnagram) table
