...
let d8input = """d dec 461 if oiy <= 1
...
type Instruction = {
    ...
    conditionOperator: int -> int -> bool;
    conditionOperand: int
    }

let convertToOperator op = 
    match op with
    | "inc" -> (fun a b -> a+b)
    | "dec" -> (fun a b -> a-b)
    
let convertToCondition op =
    match op with
    | "<" -> (fun a b -> a < b)
    | ">" -> (fun a b -> a > b)
    | "<=" -> (fun a b -> a <= b)
...
let parseLine = 
    pipe2 
        (tuple3 
            (parsename .>> spaces)
            ((pstring "inc" <|> pstring "dec") .>> spaces) 
            (pint32 .>> spaces)) 
        (tuple3 
            (pstring "if" >>. spaces >>. parsename .>> spaces) 
            ((pstring "==" <|> pstring "!=" <|> pstring "<=" <|> pstring ">=" <|> pstring "<" <|> pstring ">" ) .>> spaces) 
            (pint32))
        (fun (target, operator, operand) (conditionTarget, conditionOperator, conditionOperand) -> {
                target=target;
                operator = convertToOperator operator;
                operand = int operand;
                conditionTarget = conditionTarget;
                conditionOperator = convertToCondition conditionOperator;
                conditionOperand = int conditionOperand
            })
...
let step registers (instruction : Instruction) = 
    ...
    let comparisonTarget = getOrDefault(registers, instruction.conditionTarget, 0);
    if instruction.conditionOperator comparisonTarget instruction.conditionOperand 
        then Map.add instruction.target (instruction.operator target instruction.operand) registers
        else registers
