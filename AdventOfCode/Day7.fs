...
let parse input = 
    ...
    |> Seq.map (fun line -> 
        match line with
        | Regex "([a-z]+) \((\d+)\) -> (.*)" [node; weight; children] -> {name=node; weight=(int weight); children=(children.Split(','))}
        | Regex "([a-z]+) \((\d+)\)" [node; weight; children] -> {name=node; weight=(int weight); children= Seq.empty}
    )
    |> Seq.fold (m {name; weight; children} -> Map.add m name {name=name; weight=weight;children=children})
